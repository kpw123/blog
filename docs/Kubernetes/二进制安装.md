...
* [二进制安装](#二进制安装)
   * [一、基础优化](#一基础优化)
   * [二、SSL证书](#二ssl证书)
      * [1.Master节点上创建证书颁发机构（CA）](#1master节点上创建证书颁发机构ca)
      * [2.创建根证书](#2创建根证书)
         * [1)创建请求证书的配置文件](#1创建请求证书的配置文件)
         * [2)创建根CA证书签名请求文件](#2创建根ca证书签名请求文件)
         * [3)使用请求文件生成证书](#3使用请求文件生成证书)
   * [三、部署ETCD集群](#三部署etcd集群)
      * [ETCD节点规划](#etcd节点规划)
      * [1.创建ETCD证书签名请求文件](#1创建etcd证书签名请求文件)
      * [2.使用请求文件生成证书](#2使用请求文件生成证书)
      * [3.将证书分发至ETCD服务器](#3将证书分发至etcd服务器)
      * [4.在各个节点部署ETCD](#4在各个节点部署etcd)
      * [5.用systemctl管理ETCD](#5用systemctl管理etcd)
      * [6.启动并测试](#6启动并测试)
      * [7.ETCD配置文件（可有可无）](#7etcd配置文件可有可无)
   * [四、在Master节点上创建所有模块的证书](#四在master节点上创建所有模块的证书)
      * [1.创建Master根证书](#1创建master根证书)
      * [2.创建kube-apiserver证书](#2创建kube-apiserver证书)
      * [3.创建controller-manager证书](#3创建controller-manager证书)
      * [4.创建kube-scheduler证书](#4创建kube-scheduler证书)
      * [5.创建kube-proxy证书](#5创建kube-proxy证书)
      * [6.创建操作用户证书](#6创建操作用户证书)
      * [7.为master节点分发证书](#7为master节点分发证书)
      * [8.为node节点分发证书](#8为node节点分发证书)
      * [9.为vip节点分发证书（可有可无）](#9为vip节点分发证书可有可无)
   * [五、部署master节点](#五部署master节点)
      * [1.下载组件](#1下载组件)
      * [2.分发master组件](#2分发master组件)
      * [3.配置TLS bootstrapping](#3配置tls-bootstrapping)
      * [4.创建集群配置文件](#4创建集群配置文件)
      * [5.分发配置文件到各节点](#5分发配置文件到各节点)
      * [6.部署并启动API server](#6部署并启动api-server)
      * [7.部署并启动controller-manager](#7部署并启动controller-manager)
      * [8.部署并启动kube-scheduler](#8部署并启动kube-scheduler)
   * [六、部署node节点](#六部署node节点)
      * [1.分发命令](#1分发命令)
      * [2.部署并启动kubelet](#2部署并启动kubelet)
      * [3.部署并启动kube-proxy](#3部署并启动kube-proxy)
   * [七、集群搭建](#七集群搭建)
      * [1.将kubelet加入集群](#1将kubelet加入集群)
      * [2.集群配置](#2集群配置)
   * [八、部署网络插件](#八部署网络插件)
      * [1.部署Flanneld](#1部署flanneld)
      * [2.修改docker的systemd管理脚本以让Flanneld接管docker](#2修改docker的systemd管理脚本以让flanneld接管docker)
      * [3.重启docker并开启Flanneld](#3重启docker并开启flanneld)
   * [九、部署CoreDNS解析插件](#九部署coredns解析插件)
   * [十、测试集群](#十测试集群)
